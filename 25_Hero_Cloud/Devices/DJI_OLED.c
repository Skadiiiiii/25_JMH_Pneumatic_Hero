...
void oled_showchinese(uint8_t row, uint8_t col, uint8_t *chr)
{
    uint8_t x = col * 6;
    uint8_t y = row * 12;
    uint8_t t, t1;
    uint8_t y0 = y;
		...
		for(k=0;k<HZnum;k++)
		{
			if((tfont12[k].Index[0]==*(chr))&&(tfont12[k].Index[1]==*(chr+1)))
			{
				for (t = 0; t < 22; t++)
				{
						for (t1 = 0; t1 < 8; t1++)
						{
								if(tfont12[k].Msk[t]&(0x01<<t1))
										oled_drawpoint(x, y, Pen_Write);
								else
										oled_drawpoint(x, y, Pen_Clear);
...
void oled_show_state(uint8_t row, uint8_t col,uint8_t *chr,uint8_t state)
{
	uint8_t n =0;
	...
	if(state==0)
	{
		oled_printf(row,col+1,"off");
		
	}
	else
	{
		oled_printf(row,col+1,"on");
	}
//	oled_refresh_gram();
}
...
//	oled_show_state(0,1,"dr16",DR16_Export_Data.ChassisWorkMode);
//	oled_show_state(4,1,"yaw",M6020s_Yaw.state);
//	
//	oled_show_state(4,11,"pitch",M6020s_Pitch.state);
//	oled_show_state(4,1,(uint8_t *)"shoot1",M3508s_shoot[0].state);
//	oled_show_state(4,11,(uint8_t *)"shoot2",M3508s_shoot[1].state);
//	oled_show_state(0,1,(uint8_t *)"RF60",M6020_state[RF_205_6020]);
//	oled_show_state(1,1,(uint8_t *)"LF60",M6020_state[LF_206_6020]);
//	oled_show_state(2,1,(uint8_t *)"RB60",M6020_state[RB_207_6020]);
//	oled_show_state(3,1,(uint8_t *)"LB60",M6020_state[LB_208_6020]);
//	
//	oled_show_state(0,11,(uint8_t *)"RF35",M3508_state[RF_201_3508]);
//	oled_show_state(1,11,(uint8_t *)"LF35",M3508_state[LF_202_3508]);
//	oled_show_state(2,11,(uint8_t *)"RB35",M3508_state[RB_203_3508]);
//	oled_show_state(3,11,(uint8_t *)"LB35",M3508_state[LB_204_3508]);
