...
uint8_t JudgeSystem_rxBuff[389u]; 
...
ext_LedMeaning_t Led_Cyan;
ext_LedMeaning_t Led_Black;
ext_LedMeaning_t Led_White;
...
//	ShowData.cilentData[4].graphic_name[0] = 5;
//	ShowData.cilentData[4].graphic_tpye = 0;
//	ShowData.cilentData[4].operate_tpye = 1;
//	ShowData.cilentData[4].layer = 0;
//	ShowData.cilentData[4].color = 4;
//	ShowData.cilentData[4].start_angle = 0;
//	ShowData.cilentData[4].end_angle = 0;
//	ShowData.cilentData[4].width = 1;
//	ShowData.cilentData[4].start_x = 945;
//	ShowData.cilentData[4].start_y = 680;
//	ShowData.cilentData[4].radius = 0;
//	ShowData.cilentData[4].end_x = 945;
//	ShowData.cilentData[4].end_y = 280;
	...
	datalength = sizeof(ShowData);
	HAL_UART_Transmit(&huart3, CliendTxBuffer, datalength, 0xFF);
	//HAL_UART_Transmit_DMA(&huart3,CliendTxBuffer,datalength);
}
...
void Ex_CrossHair(void)
{
	...
	//HAL_UART_Transmit_DMA(&huart3,CliendTxBuffer,crosslength);
}

void Show_CrossHair(void)
{
		Standard_CrossHair();
//	while(HAL_DMA_GetState(&hdma_usart3_tx) != HAL_DMA_BURST_STATE_READY);
		Ex_CrossHair(); //unready test
}
/*机器人状态指示*/
...
void Show_ShootLevel(void)
{
	...
	strcpy(ShowshootLv.cilentData.data, "Pitch:");
...
void ShootLv_Init(void)
{
  ...
  //  HAL_UART_Transmit(&huart3, CliendShowBuffer, ShootInit_Datalength, 0xFF);
		...
		HAL_UART_Transmit_DMA(&huart3, CliendShowBuffer, ShootInit_Datalength);
}
...
//    int_into_char(ShootLvInit.cilentData.data,Get_IMUC_Angle(Pitch));
    memcpy(CliendShowBuffer + 5,
           (uint8_t *)&ShootLvInit.CmdID,
           (sizeof(ShootLvInit.CmdID) + sizeof(ShootLvInit.dataFrameHeader) + sizeof(ShootLvInit.cilentData)));
    Append_CRC16_Check_Sum(CliendShowBuffer, sizeof(ShootLvInit));
...
void Cap_EnergyUpdata(void)
{
	...
	Cap_Energyvalue.cilentData.grapic_data_struct.start_x = 250;
	Cap_Energyvalue.cilentData.grapic_data_struct.start_y = 650;
//	int_into_char(Cap_Energyvalue.cilentData.data, SupCap.SCCM_ReceiveData.Cap_Cell);
	strcat(Cap_Energyvalue.cilentData.data, "%");
...
void Car_DistanceFrame(void)
{
	...
	HAL_UART_Transmit_DMA(&huart3, Car_DistanBuff, distanlength);
...
//		if(GetShoot_Mode() == NoFire)
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Yellow.cilentData.data, "Fric:off ");
//		}
//		else{
//			if(GetPillObstruct_Status() == 1)
//			{
//				strcpy(Led_Yellow.cilentData.data, LED_Init);
//				strcpy(Led_Yellow.cilentData.data, "Fric:Back ");
//			}
//			else if(GetShooting() == 1)
//			{
//				strcpy(Led_Yellow.cilentData.data, LED_Init);
//				strcpy(Led_Yellow.cilentData.data, "Fric:Shooting ");
//			}
//			else if(GetShoot_Mode() == Fire)
//			{
//			  strcpy(Led_Yellow.cilentData.data, LED_Init);
//				strcpy(Led_Yellow.cilentData.data, "Fric:Fire ");
//			}
//			else if(GetShoot_Mode() == Overload_Fire)
//			{
//				strcpy(Led_Yellow.cilentData.data, LED_Init);
//				strcpy(Led_Yellow.cilentData.data, "Fric:Over ");
//			} 
//		}
		
		strcpy(Led_Yellow.cilentData.data, "+-");
...
//		if(getClassMode() == ChassisTwister)
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Green.cilentData.data, "_Chassis ");
//		}
//		else if(GetSuperCap_Mode() == Cap_Enable)
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Green.cilentData.data, "SupCapt_ON ");
//		}
//		else if(getClassMode() == ChassisFollow)
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Green.cilentData.data, "Follow_Chassis ");
//		}
//		else if(getClassMode() ==  ChassisClock)
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Green.cilentData.data, "Lock_Chassis ");
//		}
//		else
//		{
//			strcpy(Led_Yellow.cilentData.data, LED_Init);
//			strcpy(Led_Green.cilentData.data, "Chassis_Lost ");
//		}

//			switch(GetPillObstruct_Status())
//			{
//				case 1:
//					strcpy(Led_Green.cilentData.data, "NO!!! ");
//				break;
//				case 0:
//					strcpy(Led_Green.cilentData.data, "yes!!! ");
//			}
...
//	if (VisionData.Monitor.OffLine)
//	{
//		strcpy(Led_Purple.cilentData.data, LED_PURPLEMeaning);
//	}
//	else
//	{
//		switch (INFANTRY.Vision_Mode)
//		{
//		case Vision_DefaultAIM:
//			strcpy(Led_Purple.cilentData.data, "VisionMode:Default ");
//			break;

//		case Vision_BIGWindmill:
//			strcpy(Led_Purple.cilentData.data, "VisionMode:WindMill ");
//			break;

//		case Vision_BASE:
//			strcpy(Led_Purple.cilentData.data, "VisionMode:Base   ");
//			break;
...
//		case Vision_Record:
//			strcpy(Led_Purple.cilentData.data, "VisionMode:Record ");
//			break;
...
//	int_into_char(Led_Pink.cilentData.data, Get_IMUC_Angle(Pitch));
	strcpy(Led_Pink.cilentData.data, "Pitch");
...
ext_shoot_data_t ext_shoot_data;
ext_bullet_remaining_t ext_bullet_remaining;
ext_custom_controller_t ext_custom_controller;
ext_remote_control_t ext_remote_control;
/**************CRC8-Start**********************/
const unsigned char CRC8_INIT = 0xff; 
const unsigned char CRC8_TAB[256] =  
{  
...
/*  
** Descriptions: CRC8 Check_Sum function  
** Input: GetSum 
** Output: True or False (CRC Verify Result)  
*/  
...
void Judge_GetMessage(uint16_t Data_Length)
{
		for (int n = 0; n < Data_Length;)
	{
		if (JudgeSystem_rxBuff[n] == JUDGE_FRAME_HEADER)
		{
			switch (JudgeSystem_rxBuff[n + 5] | JudgeSystem_rxBuff[n + 6] << 8)
			{
				case Judge_Robot_HP: //»úÆ÷ÈËÑªÁ¿Êý¾Ý		//Message From Serve
				if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_Robot_HP))
				{
					memcpy(&ext_game_robot_HP.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[32]));
					n += JudgeLength_Robot_HP;
					...
					n++;
				break;
				case Judge_Robot_State: //±ÈÈü»úÆ÷ÈË×´Ì¬
				if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_Robot_State))
				{
					memcpy(&ext_game_robot_state.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[13]));
					n += JudgeLength_Robot_State;
					Calc_Robot_ID(ext_game_robot_state.data.robot_id);
					ext_game_robot_state.InfoUpdataFlag = 1;
				}
				else
					n++;
				break;
					case Judge_Power_Heat: //ÊµÊ±¹¦ÂÊÈÈÁ¿
				if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_Power_Heat))
				{
					memcpy(&ext_power_heat_data.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[16]));
					n += JudgeLength_Power_Heat;
					...
					n++;
				break;
				case Judge_RealTime_Shoot: //ÊµÊ±Éä»÷Êý¾Ý
				if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_RealTimeShoot))
				{
					memcpy(&ext_shoot_data.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[7]));
					n += JudgeLength_RealTimeShoot;
					ext_shoot_data.InfoUpdataFlag = 1;
				}
				else
					n++;
				case Judge_Remaining_Rounds: //×Óµ¯Ê£ÓàÊý
				if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_Remaining_Rounds))
				{
					memcpy(&ext_bullet_remaining.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[6]));
					n += JudgeLength_Remaining_Rounds;
					...
					n++;
				break;
				case Judge_Custom_controller: //Custom_controller
    	if(	Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_Custom_controller))	
				{
				memcpy(&ext_custom_controller.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[30]));
					n += JudgeLength_Custom_controller;
					ext_custom_controller.InfoUpdataFlag = 1;
				}
				else
					n++;
				break;
//				case Judge_key	:
//					//if (Verify_CRC16_Check_Sum(JudgeSystem_rxBuff + n, JudgeLength_key))
//				if(1)
//				{
//					memcpy(&ext_remote_control.data.dataBuff, &JudgeSystem_rxBuff[n + 7], sizeof(uint8_t[12]));
//					n += JudgeLength_key;
//					ext_shoot_data.InfoUpdataFlag = 1;
//				}
//				else
...
void Calc_Robot_ID(uint8_t local_id)
{
	...
	if(local_id !=0 )																	
	{
		if(local_id < 10)
		{
			...
			robot_client_ID.robot_where = 0;//Robot_Red;
		...
		else
		{
			...
			robot_client_ID.robot_where = 1;//Robot_Blue;	
			robot_client_ID.local = local_id;	
