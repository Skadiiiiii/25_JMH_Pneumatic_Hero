...
void USART3_IRQHandler(void)
{
  /* USER CODE BEGIN USART3_IRQn 0 */
	...
	if(__HAL_UART_GET_FLAG(&huart3,UART_FLAG_IDLE) && __HAL_UART_GET_IT_SOURCE(&huart3,UART_IT_IDLE))
	{
		...
		DMA_Counter = __HAL_DMA_GET_COUNTER(huart3.hdmarx);
		
			//裁判系统接收
		Judge_GetMessage(JUDGESYSTEM_PACKSIZE - DMA_Counter + 2);

		//裁判系统包大小
		__HAL_DMA_SET_COUNTER(huart3.hdmarx,JUDGESYSTEM_PACKSIZE);
